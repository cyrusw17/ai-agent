#!/usr/bin/env python3
"""
FORWARD-LOOKING SIMULATION - RECOMMENDED 2% CONFIGURATION

Shows what the portfolio would look like starting 2 weeks from today
(July 30, 2026) and running for 90 days.
"""
...
print('='*80)
print('📊 RECOMMENDED STRATEGY - FORWARD SIMULATION')
print('='*80)
print()
print('Configuration: 2% Very Conservative')
print('  - Sniper Risk: 2.0%')
print('  - Background Risk: 1.0%')
print('  - Starting Date: July 30, 2026 (2 weeks from today)')
print('  - Simulation Period: 90 days')
print('  - Starting Capital: $1,000')
print()

class ForwardBacktester:
...
# Load data - start from 2 weeks from today, need historical data for indicators
print('Loading market data for simulation...')
handler = DataHandler()

# Start date: July 30, 2026 (2 weeks from today)
sim_start = datetime(2026, 7, 30)
sim_end = sim_start + timedelta(days=90)  # 90-day forward simulation

# Need historical data for indicators (6 months before sim_start)
data_start = sim_start - timedelta(days=180)
data_end = datetime.now()  # Get all available data up to now
...
for pair in pairs:
    ...
    try:
        df = handler.fetch_data(pair, data_start.strftime('%Y-%m-%d'), data_end.strftime('%Y-%m-%d'), '4h')
        ...
        df_sim = df[df.index >= pd.Timestamp(sim_start, tz=df.index.tz)]
        
        # Generate signals for simulation period
        sniper_sigs = []
        for i in range(50, len(df)):
            bar, prev = df.iloc[i], df.iloc[i-1]
            date = bar.name if isinstance(bar.name, pd.Timestamp) else pd.Timestamp(bar['date'])
            
            # Only include signals in simulation period
            if date < pd.Timestamp(sim_start, tz=df.index.tz):
                continue
            if date > pd.Timestamp(sim_end, tz=df.index.tz):
                break
...
print(f'Wins: {sum(1 for t in bt.trades if t["won"])}')
print(f'Losses: {sum(1 for t in bt.trades if not t["won"])}')
print(f'Win Rate: {sum(1 for t in bt.trades if t["won"]) / len(bt.trades) * 100:.1f}%' if len(bt.trades) > 0 else 'No trades')
print(f'Max Drawdown: {bt.max_dd:.2f}%')
print(f'Stopped Out: {bt.stopped_out}')
if bt.stopped_date:
...
viz_data = {
    ...
    'simulation_start': sim_start.strftime('%Y-%m-%d'),
    'simulation_end': sim_end.strftime('%Y-%m-%d'),
    'configuration': {
...
# Save data
with open('/workspace/docs/forward_simulation_data.json', 'w') as f:
    json.dump(viz_data, f, indent=2)

with open('/workspace/forward_simulation_data.json', 'w') as f:
    json.dump(viz_data, f, indent=2)
