#!/usr/bin/env python3
"""
$1000 LEVERAGED ACCOUNT WITH 20% MAX DRAWDOWN LIMIT

Conservative simulation with strict risk control:
- Starting capital: $1000
- Leverage: 50:1
- Max drawdown: 20% ($200 max loss)
- Stop trading if 20% DD reached
"""
...
print('='*80)
print('💰 $1000 LEVERAGED ACCOUNT - 20% MAX DRAWDOWN')
print('='*80)
print()
print('Configuration:')
...
print('  - Stop Trading: If DD reaches 20%')
print('  - Strategy: Dynamic Targets + Volatility Regime')
print()

class ConservativeLeveragedBacktester:
    """Leveraged account with strict 20% DD limit"""
    
    def __init__(self, capital=1000, leverage=50, max_dd_pct=20):
        self.capital = capital
        self.init = capital
        self.leverage = leverage
        self.max_dd_pct = max_dd_pct
        self.max_dd_dollar = capital * (max_dd_pct / 100)  # $200
...
# Run multiple simulations to get average
print('='*80)
print('RUNNING 20 SIMULATIONS WITH 20% DD LIMIT')
print('='*80)
print()
...
for sim_num in range(20):
    ...
    bt = ConservativeLeveragedBacktester(capital=1000, leverage=50, max_dd_pct=20)
...
# Re-run the median simulation to get detailed data
bt_viz = ConservativeLeveragedBacktester(capital=1000, leverage=50, max_dd_pct=20)

np.random.seed(median_idx)  # Use same seed as median result
...
with open('/workspace/docs/leveraged_conservative_data.json', 'w') as f:
